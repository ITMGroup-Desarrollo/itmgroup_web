  ...
  <div class="content">
    <div class="col1 col wow fadeInUp" data-wow-delay='0.2s'>
      ...
      <?php if (Lang\getLang() == 'en') { ?>
        ...
        <p class="option brfooter<?= (Util\getPageName() == 'brands') ? 'selected' : '' ?> ">
          <a href="javascript:void(0);" onclick="scrollToSection('menu')"><span>Brands</span></a>
        </p>
      ...
      <?php } else { ?>
        ...
        <p class="option brfooter<?= (Util\getPageName() == 'marcas') ? 'selected' : '' ?> ">
          <a href="javascript:void(0);" onclick="scrollToSection('menu')"><span>Marcas</span></a>
        </p>
    ...
    <div class="col2 col wow fadeInUp" data-wow-delay='0.0s'>
      <a href="javascript:void(0);" onclick="scrollToSection()" aria-label="inicio">
    ...
    <div class="col3 col wow fadeInUp" data-wow-delay='0.2s'>
      <a class="icon" aria-label="facebook" target="_blank" rel="nofollow"
        href="https://www.facebook.com/share/16G4YLph8q/">
    ...
    <div class="bottom">
      <p class="text"><b>ITM GROUP <?php echo date("Y"); ?></b> <br>
        <?php if (Lang\getLang() == 'en') { ?>
          All rights reserved
        <?php } else { ?>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    const brandsFooter = document.querySelector('.brfooter'); // El botón de activación
    const subMenu = document.querySelector('.idtx-items'); // El submenú

    const showSubMenufooter = () => {
      subMenu.style.marginTop = '20px';
      subMenu.style.marginBottom = '-10px';
      subMenu.style.height = '100%';
      subMenu.style.display = 'block';
      // Indica que el clic fue desde el footer
      window.isFooterClicked = true;
    };
    // Mostrar el submenú al hacer clic en brandsFooter
    brandsFooter.addEventListener('click', (event) => {
  ...
  function smoothScrollTo(targetPosition) {
    ...
    const step = (targetPosition - currentPosition) / 50;
    ...
    function scrollStep() {
      ...
      if ((step > 0 && currentPosition < targetPosition) || (step < 0 && currentPosition > targetPosition)) {
        ...
        window.scrollTo(0, targetPosition); // Asegura la posición final
      }
    }
  ...
  function scrollToSection(sectionId) {
    ...
    if (section) {
      const topPosition = section.offsetTop + 101;
      brandsElement.classList.add('focused');
      smoothScrollTo(topPosition);

      // Verifica si es un dispositivo móvil y ejecuta openSmallMenu()
      if (window.innerWidth <= 768) { // 768px es un tamaño típico para tabletas y móviles
        openSmallMenu();
      }
    } else {
      window.scrollTo({
